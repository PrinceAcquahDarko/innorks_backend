...
const registerUser = async(req, res) => {
    ...
    try {
        let registerUser =  await con.query('SELECT * FROM bookusers WHERE email = $1', [data.email]);

        if (registerUser.rows.length) return res.status(404).send({ message:"user already exists"});
     
        data.password = hashPassword(data.password);
        
        let user = await con.query('INSERT INTO bookusers (firstname, lastname,email, password) VALUES ($1,$2,$3,$4) RETURNING *', [data.firstname, data.lastname, data.email, data.password])
